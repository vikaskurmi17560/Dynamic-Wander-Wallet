...
function profile() {
  ...
  const { name, profile, banner, bio , phone_no } = useData();
  ...
  return (
    ...
    <main className='w-full min-h-screen flex flex-col bg-slate-200  '>
      <section className='flex flex-col gap-24'>
        <section style={{ backgroundImage: `url(${banner})`}} className="relative w-[100%] bg-cover bg-center pl-6 h-60 bg-slate-600 flex items-center ">
        {profile ? <img  src={profile} alt='profile image' className="w-[200px] absolute top-[50%] h-[200px] bg-slate-300 rounded-full flex justify-center items-center" />
        :
        ...
        <section className="w-full h-fit flex flex-row gap-4 bg-gray-400 justify-around p-4">
          ...
          <div className="w-[35%] h-fit flex flex-col bg-gray-100 p-4 gap-4">
            ...
            <div className="font-bold text-xl text-black">:</div>
            <div className="text-lg">{phone_no}</div>
          </div>
